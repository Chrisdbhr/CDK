	...
	public class CPlayerAttack : MonoBehaviour {
		...
		private void Update() {
			if (Input.GetButtonDown(CInputKeys.ATTACK) && 
				...
				if (this._inventory.EquippedWeapon is CGunData gun) {
					if (!gun.HasAmmo()) {
						// no ammo
					...
					if (usedAmmo.ScriptableObject is CAmmoScriptableObject ammo) {
						switch (ammo.CProjectileType) {
							case CProjectileType.raycast:
								...
								if (hitSomething) {
									...
									// apply damage
									hitInfo.collider.SendMessage(nameof(CIDamageable.TakeDamage), ammo.HitInfo, SendMessageOptions.DontRequireReceiver);

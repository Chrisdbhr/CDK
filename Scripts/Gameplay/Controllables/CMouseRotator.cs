	...
	public class CMouseRotator : MonoBehaviour {
		...
		private void Update() {
			...
			this._transform.Rotate(Vector3.up,
				this._inputLook.x * (Screen.width <= 0 ? 1f : (float)Screen.width) * this._rotationSpeed.x * Time.deltaTime,
				Space.World );
			this._transform.Rotate(this._transform.right,
				this._inputLook.y * (Screen.height <= 0 ? 1f : (float)Screen.height) * -1 * this._rotationSpeed.y * Time.deltaTime, 
				Space.World );

...
namespace CDK {
	public class CInventoryViewItemOptionsMenu : MonoBehaviour {
		...
		[NonSerialized] private CItemBaseData _item;
		...
		public void Open(CItemBaseData itemBase, int itemIndex, Vector2 position) {
			this._viewItensParent.transform.position = position;
			
			this._item = itemBase;
			this._itemIndex = itemIndex;
			Debug.Log($"Opening {itemBase.ScriptableObject.name} options. Itemindex is {itemIndex}.");
			
			// set visible buttons
			this._useItemButton.gameObject.SetActive(itemBase.ScriptableObject.IsConsumable());
			this._equipItemButton.gameObject.SetActive(itemBase.ScriptableObject is CWeaponBaseScriptableObject);
			...
			this._discardItemButton.gameObject.SetActive(itemBase.ScriptableObject.CanBeDropped);

...
namespace CDK {
	public class CInventory : MonoBehaviour {
		...
		[field: NonSerialized]
		public CItemBaseData[] InventoryItems { get; private set; }
		...
		[NonSerialized] private ReactiveProperty<CWeaponBaseData> _equippedWeaponRx;
		[NonSerialized] private CWeaponBaseScriptableObject[] _quickSlotWeapons = new CWeaponBaseScriptableObject[4];
		...
		private void OnEnable() {
			...
			this._equippedWeaponRx.Subscribe(newWeapon => {
				...
				if (newWeapon.ScriptableObject is CWeaponBaseScriptableObject weaponScripObj) {
					...
					Debug.LogError($"New weapon is not a weapon of type {nameof(CWeaponBaseScriptableObject)}");
				}
		...
		/// </summary>
		public bool AddItem(CItemBaseData item, bool hideNotification = true) {
			if (item == null) {
			...
			for (int i = 0; i < this.InventoryItems.Length; i++) {
				...
				alreadyHasThisItem = this.InventoryItems[i].ScriptableObject == item.ScriptableObject;
				if (alreadyHasThisItem) {
					// item exists, increase it quantity
					this.InventoryItems[i].Count += item.Count;
					this.UpdateView();
					return true;
				}
			}
			...
			for (int i = 0; i < this.InventoryItems.Length; i++) {
				...
				if (item is CGunData gun) {
					// auto equip gun
					Debug.Log($"Auto equipping collected gun.");
					this._equippedWeaponRx.Value = gun;
				}
		...
		public void UseOrEquipItem(int itemIndex) {
			...
			if (item is CGunData weapon) {
				// equip
				this._equippedWeaponRx.Value = weapon;
			}
			else {
				// use
				print($"TODO implement usage of item {this.InventoryItems[itemIndex].ScriptableObject.ItemName}");
			}
		...
		public  void ExamineItem(int itemIndex) {
			print($"TODO implement Examine on item {this.InventoryItems[itemIndex].ScriptableObject.ItemName}");
			this.UpdateView();
		}
		...
		public void DiscardOrDropItem(int itemIndex, bool drop = false) {
			if (drop) {
				...
				var droppedItem = Instantiate(this.InventoryItems[itemIndex].ScriptableObject.ItemMeshWhenDropped, posToDrop, this.InventoryItems[itemIndex].ScriptableObject.ItemMeshWhenDropped.transform.rotation);
			...
			if (this.InventoryItems[itemIndex] as CGunData == this.EquippedWeapon) { 
		...
		public CAmmoData GunShootConsumeAmmo() {
			var weapon = this.EquippedWeapon as CGunData;
			if (weapon == null) {
				Debug.LogError($"Could not fire Weapon because it is not a Gun!");
				return null;
			}

			if (weapon.EquippedAmmoData.Count <= 0) {
